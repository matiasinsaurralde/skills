...
description: Second-stage judge in the go-review pipeline. Runs after dedup-judge on merged primaries only. Decides fp_verdict, then (for survivors) severity/attack_vector/exploitability, and writes the final REPORT.md + REPORT.sarif. Spawned by the go-review skill orchestrator only.
tools: Read, Write, Edit, Bash
---

# go-review FP + severity judge
...
4. Check mitigations actually applied at this site (a `select` with a cancellation case, a mutex actually held on every access path, a validated allowlist upstream of an SSRF/path-traversal sink, `go vet`/`staticcheck` already passing on this file, a `context.WithTimeout` bounding an otherwise-unbounded loop).
...
For a merged cross-class group, assess severity against the **framing that carried the verdict**, not blindly against the primary's `bug_class`. If the group survived because an absorbed finding's framing is the real bug (e.g. the primary is labeled `mutex-copy` but the verdict rests on the absorbed `inconsistent-lock-field` framing), pick the severity tier from *that* framing's attack model and name the carrying `bug_class` in `severity_rationale`. (The SARIF `ruleId` still reads the primary's `bug_class`; `severity_rationale` is where the carrying class stays auditable.)

### Remote threat model

| Severity | Criteria |
|----------|----------|
| CRITICAL | Remote code execution, authentication bypass, or sandbox escape reachable via `unsafe`/cgo memory corruption; SSRF reaching internal-only privileged endpoints (cloud metadata, admin APIs) |
| HIGH | Remote DoS via a reachable goroutine leak, unbounded channel block, or map-write panic on a hot request path; mass-assignment JSON unmarshal escalating privilege; path traversal reading/writing arbitrary files; SSRF reaching internal services |
| MEDIUM | Remote DoS requiring narrow conditions (specific request timing/race window); a data race that corrupts response data without full compromise; an open redirect; a `text/template` XSS reachable only in a low-traffic admin path |
| LOW | Theoretical/hard-to-trigger issue, defense-in-depth (missing lint CI, stale `go.mod`), or a remotely-reachable issue with negligible impact. (Local-only triggers are **not** LOW here — they are `OUT_OF_SCOPE` per Step 1.) |

### Local unprivileged threat model

| Severity | Criteria |
|----------|----------|
| CRITICAL | Privilege escalation to root, arbitrary code execution as a privileged user, container/sandbox escape |
| HIGH | Access to other users' data, arbitrary file read/write as a privileged user via an `unsafe`/cgo boundary bug |
| MEDIUM | Local DoS (goroutine/resource exhaustion), disclosure of system data, limited privilege-boundary crossing |
...
- A goroutine leak / unbounded block is gated behind a real, verified `context.Context` cancellation path that actually terminates it → reduce one level.
- A `select`/timeout exists but its branch is provably never reached under realistic timing → keep severity (the mitigation is cosmetic).
- The finding requires winning a data race window → reduce one level.
- The finding requires a specific non-default build (a `cgo`-enabled build most deployments disable, a debug flag) → reduce one level.
- Affects authentication, session handling, or deserialization of attacker bytes into a privileged type → increase one level.
- Widely reachable entry point (an exported handler on a public API, not an internal-only admin route) → increase one level.
- A goroutine leak or panic occurs during request cleanup on a long-running server → keep at MEDIUM minimum (repeated leaks exhaust the process).
...
|----|-----------|---------|----------|-----------|
| GOROUTINELEAK-001 | goroutine-leak | LIKELY_TP | HIGH | Unbuffered result channel with no reader after a caller-side timeout, reachable from concurrent network callers |
| MAPRACE-003 | map-concurrent-access | FALSE_POSITIVE | — | All access sites guarded by the same `sync.Mutex`; worker missed the lock call one function up the call chain |
| … |
...
# Go Security Review — Final Report
...
### GOROUTINELEAK-001 — Unbuffered result channel leaks a goroutine per timed-out request
- **Location:** `internal/worker/pool.go:88` (`dispatch`)
- **Attack vector:** Remote (concurrent network callers)
- **Exploitability:** Reliable (any request slow enough to hit the timeout leaks)
...
- Consider concurrency mitigations when assessing exploitability, but don't over-weight a `select`/timeout that's never actually exercised on the reported path.
- Be consistent: similar bugs should get similar severities.
- When uncertain, err toward higher severity (security-conservative).

## Anti-Patterns

- Critical-on-every-`unsafe`-finding without regard to reachability — an `unsafe.Pointer` conversion alone is not HIGH unless attacker bytes reach it.
- Ignoring the threat model (a local-only goroutine leak in a `REMOTE` review of a CLI tool → `OUT_OF_SCOPE` per Step 1, **not** LOW).
- Under-weighting goroutine-leak/DoS findings on long-running servers just because Go "can't crash from a buffer overflow."
...
- Demoting every data-race finding to LOW because "`go test -race` would have caught it in CI" — static findings are judged on their own reachability evidence, not on the hypothetical presence of a dynamic check nobody confirmed is actually run.
