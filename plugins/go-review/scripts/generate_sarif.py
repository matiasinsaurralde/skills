...
# Go bug_class slugs from prompts/clusters/manifest.json.
RULE_DESCRIPTIONS = {
    # error-handling
    "error-ignored": "Security-relevant error return discarded or ignored",
    "error-sentinel-compare": "Sentinel error compared with == instead of errors.Is/As",
    "error-wrap-lost-type": "Error wrapping loses type information needed downstream",
    "panic-recover-misuse": "recover() outside defer, or swallows a corrupted-state panic",
    # concurrency-goroutines
    "goroutine-leak": "Spawned goroutine has no reachable cancellation or exit path",
    "channel-deadlock": "Mismatched channel send/receive counts deadlock",
    "waitgroup-race": "sync.WaitGroup.Add called inside the goroutine instead of before go",
    "channel-close-misuse": "Channel closed more than once, or send on a closed channel",
    "channel-unbounded-block": "Channel operation can block forever with no select/timeout",
    # concurrency-data-race
    "mutex-copy": "sync.Mutex/RWMutex copied by value after first use",
    "inconsistent-lock-field": "Shared field read/written outside the mutex guarding its siblings",
    "lazy-init-race": "Lazy initialization race without sync.Once",
    "map-concurrent-access": "Concurrent map read/write without synchronization",
    "atomic-nonatomic-mix": "sync/atomic value also read or written non-atomically elsewhere",
    # nil-interface-pitfalls
    "typed-nil-interface": "Typed nil stored in an interface makes != nil true",
    "nil-map-write": "Write to a nil map panics on an attacker-reachable path",
    "nil-pointer-deref": "Nil pointer dereference on an unchecked optional field",
    # defer-closure-hazards
    "loop-var-capture": "Closure or goroutine captures a pre-1.22 shared loop variable",
    "defer-in-loop": "defer inside a loop accumulates unbounded resource holds",
    "defer-order-hazard": "defer runs before a required nil/err check, or in the wrong order",
    "defer-error-discarded": "Deferred call's error is silently discarded",
    # static-hygiene
    "gomod-version-stale": "go.mod go directive is missing or far behind the toolchain",
    "lint-config-missing": "go vet/staticcheck not wired into CI",
    "deprecated-api": "Deprecated stdlib API in use (e.g. io/ioutil post-1.16)",
    # unsafe-boundary
    "unsafe-pointer-conversion": "unsafe.Pointer conversion outside the documented valid patterns",
    "unsafe-len-cap": "unsafe.Slice/unsafe.String length driven by attacker-controlled input",
    "unsafe-layout-assumption": "unsafe.Offsetof/struct layout assumption broken by field changes",
    "uintptr-retained": "uintptr retained across a GC safepoint; pointer may move or be collected",
    # context-lifecycle
    "context-cancel-leak": "context.WithCancel/WithTimeout cancel func never called on all paths",
    "context-key-collision": "context.Value key uses a built-in type instead of an unexported type",
    "context-stored-in-struct": "context.Context stored in a struct field instead of threaded as a param",
    "context-done-ignored": "Long-running loop never checks ctx.Done()/ctx.Err()",
    # web-input-handling
    "ssrf": "Attacker-controlled URL reaches an outbound HTTP request without an allowlist",
    "path-traversal": "Request-derived path segment escapes the intended directory",
    "open-redirect": "Unchecked user-controlled redirect target",
    "template-autoescape-bypass": "text/template used for HTML output loses auto-escaping",
    "http-no-timeout": "http.Client/http.Server missing timeouts (slowloris/resource exhaustion)",
    # deserialization-safety
    "json-mass-assignment": "Exported struct field is attacker-settable via encoding/json Unmarshal",
    "xml-entity-expansion": "Custom XML entity map reintroduces external entity expansion",
    "gob-arbitrary-type": "encoding/gob decodes attacker bytes into an interface type",
    "recursive-decode-stack-overflow": "Unbounded recursion in a custom Unmarshal on nested input",
}
...
def build_sarif(output_dir: Path) -> dict[str, Any]:
    ...
    for finding in findings:
        ...
        title = str(finding.get("title") or finding.get("id") or "go-review finding")
        markers: list[str] = []
        ...
        "runs": [
            {
                "tool": {
                    "driver": {
                        "name": "go-review",
                        "informationUri": "https://github.com/trailofbits/skills/tree/main/plugins/go-review",
                        "rules": rules,
                    }
                },
                ...
                "originalUriBaseIds": {
                    "%SRCROOT%": {
                        "description": {
                            "text": "Root of the audited Go module or workspace; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate REPORT.sarif for a go-review output dir"
    )
