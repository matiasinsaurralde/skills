...
Usage:
    python3 build_run_plan.py \\
        --plugin-root /abs/plugins/go-review \\
        --output-dir /abs/.go-review-results/<ts> \\
        --threat-model REMOTE \\
        --severity-filter medium \\
        --scope-subpath src \\
        --context-roots . \\
        --has-unsafe false --has-cgo false --has-concurrency true \\
        --has-context true --has-reflection false --has-net-http true \\
        --has-serialization true --has-crypto false
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        required=True,
        type=Path,
        help="Absolute path to the go-review plugin root (contains prompts/clusters/manifest.json)",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help=(
            ...
            "spawned as its own go-review-worker with a -{i}-suffixed cluster_id. Clusters "
...
def split_oversized_clusters(
    ...
    **Consolidated clusters (`consolidated: true`) are never chunked**, regardless
    ...
    for cluster in selected:
        ...
        # workers skip in practice — see go-review-worker.md's chunked-subset rule).
...
def _render_shared_prefix_lines(
    *,
    ...
    """
    lines: list[str] = []
    lines.append("You are a go-review worker on a parallel Go security review.")
