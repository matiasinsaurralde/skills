"""Gating and per-pass filtering tests for build_run_plan.build_selection.

These run against the real cluster manifest and plugin root so the prompt-path
existence checks inside build_selection() resolve.
"""

from __future__ import annotations

import json
from pathlib import Path
from typing import Any

import pytest

import build_run_plan
from build_run_plan import build_selection

PLUGIN_ROOT = Path(__file__).resolve().parent.parent
MANIFEST_PATH = PLUGIN_ROOT / "prompts" / "clusters" / "manifest.json"


def load_manifest() -> dict[str, Any]:
    return json.loads(MANIFEST_PATH.read_text())


def make_flags(
    *,
    has_unsafe: bool = False,
    has_cgo: bool = False,
    has_concurrency: bool = False,
    has_context: bool = False,
    has_reflection: bool = False,
    has_net_http: bool = False,
    has_serialization: bool = False,
    has_crypto: bool = False,
) -> dict[str, bool]:
    return {
        "has_unsafe": has_unsafe,
        "has_cgo": has_cgo,
        "has_concurrency": has_concurrency,
        "has_context": has_context,
        "has_reflection": has_reflection,
        "has_net_http": has_net_http,
        "has_serialization": has_serialization,
        "has_crypto": has_crypto,
    }


def select(flags: dict[str, bool], threat_model: str = "REMOTE") -> list[dict[str, Any]]:
    return build_selection(
        load_manifest(),
        plugin_root=PLUGIN_ROOT,
        flags=flags,
        threat_model=threat_model,
    )


def cluster_by_id(selected: list[dict[str, Any]], cid: str) -> dict[str, Any] | None:
    for c in selected:
        if c["cluster_id"] == cid:
            return c
    return None


def prefixes(cluster: dict[str, Any]) -> list[str]:
    return [p["prefix"] for p in cluster["passes"]]


def test_unsafe_boundary_selected_only_with_has_unsafe():
    off = select(make_flags())
    assert cluster_by_id(off, "unsafe-boundary") is None

    on = select(make_flags(has_unsafe=True))
    unsafe = cluster_by_id(on, "unsafe-boundary")
    assert unsafe is not None
    assert prefixes(unsafe) == ["UNSAFECONV", "UNSAFELEN", "UNSAFELAYOUT", "UINTPTRUNSAFE"]


def test_context_lifecycle_gated_on_has_context():
    off = select(make_flags())
    assert cluster_by_id(off, "context-lifecycle") is None

    on = select(make_flags(has_context=True))
    ctx = cluster_by_id(on, "context-lifecycle")
    assert ctx is not None
    assert prefixes(ctx) == ["CTXLEAK", "CTXKEYTYPE", "CTXSTORED", "CTXIGNORED"]


def test_web_input_handling_gated_on_has_net_http():
    off = select(make_flags())
    assert cluster_by_id(off, "web-input-handling") is None

    on = select(make_flags(has_net_http=True))
    web = cluster_by_id(on, "web-input-handling")
    assert web is not None
    assert prefixes(web) == ["SSRF", "PATHTRAVERSAL", "OPENREDIRECT", "TEMPLATEXSS", "HTTPNOTIMEOUT"]


def test_deserialization_safety_gated_on_has_serialization():
    off = select(make_flags())
    assert cluster_by_id(off, "deserialization-safety") is None

    on = select(make_flags(has_serialization=True))
    deser = cluster_by_id(on, "deserialization-safety")
    assert deser is not None
    assert prefixes(deser) == ["JSONMASSASSIGN", "XMLENTITY", "GOBTYPE", "RECURSEDECODE"]


def test_concurrency_clusters_gated_on_has_concurrency():
    off = select(make_flags())
    assert cluster_by_id(off, "concurrency-goroutines") is None
    assert cluster_by_id(off, "concurrency-data-race") is None

    on = select(make_flags(has_concurrency=True))
    goroutines = cluster_by_id(on, "concurrency-goroutines")
    race = cluster_by_id(on, "concurrency-data-race")
    assert goroutines is not None
    assert race is not None
    assert prefixes(goroutines) == [
        "GOROUTINELEAK",
        "CHANDEADLOCK",
        "WGRACE",
        "CHANCLOSE",
        "CHANBLOCK",
    ]
    assert prefixes(race) == ["LOCKCOPY", "RACEFIELD", "LAZYINITRACE", "MAPRACE", "ATOMICMIX"]


def test_always_on_clusters_present_regardless_of_flags():
    selected = select(make_flags())
    for cid in (
        "error-handling",
        "nil-interface-pitfalls",
        "defer-closure-hazards",
        "static-hygiene",
    ):
        assert cluster_by_id(selected, cid) is not None, cid


def test_new_gates_are_isolated():
    unsafe_only = select(make_flags(has_unsafe=True))
    assert cluster_by_id(unsafe_only, "unsafe-boundary") is not None
    assert cluster_by_id(unsafe_only, "context-lifecycle") is None
    assert cluster_by_id(unsafe_only, "web-input-handling") is None

    context_only = select(make_flags(has_context=True))
    assert cluster_by_id(context_only, "context-lifecycle") is not None
    assert cluster_by_id(context_only, "unsafe-boundary") is None


def test_manifest_gates_and_requires_are_known():
    manifest = load_manifest()
    for cluster in manifest["clusters"]:
        assert cluster["gate"] in build_run_plan.GATE_VALUES, cluster["cluster_id"]
        for p in cluster.get("passes", []):
            for req in p.get("requires", []) or []:
                assert req in build_run_plan.KNOWN_REQUIRES, (cluster["cluster_id"], p)


def test_flag_vocabularies_derive_from_capability_flags():
    assert build_run_plan.GATE_VALUES == {"always", *build_run_plan.CAPABILITY_FLAGS}
    assert build_run_plan.KNOWN_REQUIRES == set(build_run_plan.CAPABILITY_FLAGS)
    assert set(make_flags()) == set(build_run_plan.CAPABILITY_FLAGS)


def test_rendered_prefix_lists_every_capability_flag():
    lines = build_run_plan._render_shared_prefix_lines(
        output_dir=Path("/tmp/out"),
        scope_root=".",
        context_roots=".",
        threat_model="REMOTE",
        severity_filter="all",
        flags=make_flags(has_unsafe=True, has_net_http=True),
        context_md_body="ctx",
    )
    codebase = next(line for line in lines if line.startswith("Codebase: "))
    for flag in build_run_plan.CAPABILITY_FLAGS:
        assert f"{flag}=" in codebase


def test_full_run_all_flags_end_to_end_snapshot():
    """Golden snapshot of the production fan-out: real manifest, every capability
    flag on, default --max-passes-per-worker 4. Locks the selection->split seam
    (worker count with these exact chunk ids/sizes), so a manifest edit that grows
    a cluster, or a regression in the selection->split pipeline, cannot silently
    change worker count or chunk ids without failing a test. The two consolidated
    clusters (unsafe-boundary, concurrency-goroutines) are never chunked — one
    worker each, full pass list."""
    flags = make_flags(
        has_unsafe=True,
        has_cgo=True,
        has_concurrency=True,
        has_context=True,
        has_reflection=True,
        has_net_http=True,
        has_serialization=True,
        has_crypto=True,
    )
    selected = build_selection(
        load_manifest(), plugin_root=PLUGIN_ROOT, flags=flags, threat_model="REMOTE"
    )
    split = build_run_plan.split_oversized_clusters(selected, max_passes=4)

    expected_ids = [
        "error-handling",
        "concurrency-goroutines",  # consolidated -> not chunked (5 passes, one worker)
        "concurrency-data-race-1",
        "concurrency-data-race-2",
        "nil-interface-pitfalls",
        "defer-closure-hazards",
        "static-hygiene",
        "unsafe-boundary",  # consolidated -> not chunked (4 passes, one worker)
        "context-lifecycle",
        "web-input-handling-1",
        "web-input-handling-2",
        "deserialization-safety",
    ]
    expected_sizes = [4, 5, 4, 1, 3, 4, 3, 4, 4, 4, 1, 4]

    assert [c["cluster_id"] for c in split] == expected_ids
    assert [len(c["passes"]) for c in split] == expected_sizes
    assert len(split) == 12
    assert sum(len(c["passes"]) for c in split) == 41  # total bug-class passes unchanged


def test_non_remote_threat_models_select():
    """The non-REMOTE threat models must also drive a valid selection. No pass in
    the manifest is threat-model-gated today, so the cluster set matches REMOTE —
    this exercises the LOCAL_UNPRIVILEGED / BOTH code path that no other test hits."""
    flags = make_flags(has_unsafe=True, has_concurrency=True, has_net_http=True)
    remote = [c["cluster_id"] for c in select(flags, threat_model="REMOTE")]
    assert [c["cluster_id"] for c in select(flags, threat_model="LOCAL_UNPRIVILEGED")] == remote
    assert [c["cluster_id"] for c in select(flags, threat_model="BOTH")] == remote


def test_skip_threat_models_filters_pass(tmp_path):
    """pass_filtered_out drops a pass whose skip_threat_models includes the active
    threat model, and keeps it under another model."""
    prompt = tmp_path / "prompts" / "clusters" / "c.md"
    prompt.parent.mkdir(parents=True)
    prompt.write_text("# c\n", encoding="utf-8")
    sub = tmp_path / "prompts" / "general" / "p.md"
    sub.parent.mkdir(parents=True)
    sub.write_text("# p\n", encoding="utf-8")
    manifest = {
        "version": 1,
        "clusters": [
            {
                "cluster_id": "c",
                "prompt": "prompts/clusters/c.md",
                "consolidated": False,
                "gate": "always",
                "passes": [
                    {
                        "bug_class": "remote-only",
                        "prefix": "RO",
                        "prompt": "prompts/general/p.md",
                        "skip_threat_models": ["LOCAL_UNPRIVILEGED"],
                    },
                    {"bug_class": "always", "prefix": "AL", "prompt": "prompts/general/p.md"},
                ],
            }
        ],
    }
    flags = make_flags()
    local = build_selection(
        manifest, plugin_root=tmp_path, flags=flags, threat_model="LOCAL_UNPRIVILEGED"
    )
    remote = build_selection(manifest, plugin_root=tmp_path, flags=flags, threat_model="REMOTE")
    local_c = cluster_by_id(local, "c")
    remote_c = cluster_by_id(remote, "c")
    assert local_c is not None and remote_c is not None
    assert prefixes(local_c) == ["AL"]
    assert prefixes(remote_c) == ["RO", "AL"]


def _render_cluster(cluster: dict[str, Any]) -> str:
    return build_run_plan.render_worker_prompt(
        worker_n=1,
        cluster=cluster,
        output_dir=Path("/tmp/out"),
        scope_root=".",
        context_roots=".",
        threat_model="REMOTE",
        severity_filter="all",
        flags=make_flags(has_unsafe=True, has_net_http=True),
        context_md_body="ctx",
    )


def test_spawn_prompt_consolidated_omits_subprompt_section():
    """Contract lock (worker.md self-check): a consolidated cluster's spawn prompt
    must NOT render a 'Sub-prompt paths:' section at all — its absence is
    well-formed, not a missing required field."""
    prompt = _render_cluster(
        {
            "cluster_id": "unsafe-boundary",
            "consolidated": True,
            "cluster_prompt": "/abs/unsafe-boundary.md",
            "passes": [{"bug_class": "unsafe-pointer-conversion", "prefix": "UNSAFECONV"}],
        }
    )
    assert "Sub-prompt paths:" not in prompt
    assert "Pass bug classes: unsafe-pointer-conversion" in prompt
    assert "Pass prefixes: UNSAFECONV" in prompt
    assert "Skip subclasses: (none)" in prompt


def test_spawn_prompt_nonconsolidated_includes_subprompt_section():
    prompt = _render_cluster(
        {
            "cluster_id": "concurrency-data-race",
            "consolidated": False,
            "cluster_prompt": "/abs/concurrency-data-race.md",
            "passes": [
                {
                    "bug_class": "mutex-copy",
                    "prefix": "LOCKCOPY",
                    "prompt": "/abs/general/mutex-copy-finder.md",
                }
            ],
        }
    )
    assert "Sub-prompt paths:" in prompt
    assert "  - /abs/general/mutex-copy-finder.md" in prompt
    assert "Skip subclasses: (none)" in prompt


def test_spawn_prompt_codebase_line_is_comma_separated():
    """Contract lock (worker.md illustration): the Codebase flags line is
    comma-separated."""
    prompt = _render_cluster(
        {
            "cluster_id": "error-handling",
            "consolidated": False,
            "cluster_prompt": "/abs/error-handling.md",
            "passes": [
                {
                    "bug_class": "error-ignored",
                    "prefix": "ERRIGN",
                    "prompt": "/abs/general/error-ignored-finder.md",
                }
            ],
        }
    )
    codebase = next(line for line in prompt.splitlines() if line.startswith("Codebase: "))
    assert ", " in codebase
    assert codebase.startswith("Codebase: has_unsafe=true, has_cgo=")


if __name__ == "__main__":
    import sys

    raise SystemExit(pytest.main([__file__, *sys.argv[1:]]))
